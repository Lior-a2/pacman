...
bool Controller::run(sf::RenderWindow & window,int highScores[],sf::Music & backroundMusic,int & i)
{
	...
	while (window.isOpen())
	{
		...
		timer.restart();
		static sf::Time two_min = sf::seconds(120);
		
		
		while (!isOver())
		{
			m_pacman.m_curr_time = two_min.asSeconds() - timer.getElapsedTime().asSeconds();
			if (m_pacman.m_curr_time == 0)
			{
				break;
			}
			...
			draw(window, passedTime, m_pacman.m_curr_time);
			window.display();
			window.clear();
		}
		break;
...
void Controller::moveDynamic(sf::RenderWindow & window, float passedTime, bool & isMoving)
{
	...
	Tree = getBfsTree(sf::Vector2i((int)(m_pacman.getPosition().x / P_SIZE),
		(int)(m_pacman.getPosition().y / P_SIZE)), m_level.getBoard());
...
// this function resets the level value ...
